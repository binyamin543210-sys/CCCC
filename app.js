// BNAPP Calendar – LUX V6
// כולל: לוח עברי/לועזי, שבת מחושבת אסטרונומית, חגים, Firebase, חיפוש, עיר, Waze, Dark/Light
...
function initFirebase() {
  if (!window.firebase || !window.firebaseConfig) {
    console.warn("Firebase SDK או config חסר – סנכרון כבוי");
    return;
  }
  try {
...
function updateMonthHeaders() {
  ...
  if (!hInfo) hInfo = CAL.hebrewDates[dateKey(new Date(y, m, 1))];
...
async function searchCityGlobal(query) {
  ...
  const res = await fetch(url, {
    headers: { "User-Agent": "BNAPP-Calendar" }
  });
...
async function reloadCityDependentData() {
  ...
  computeShabbatForMonth(CAL.viewYear, CAL.viewMonth);
  renderMonth();
}

// ---------- Shabbat (SunCalc, ללא Hebcal) ----------
function computeShabbatForMonth(year, month) {
  const { lat, lon } = CAL.cityInfo;
  if (!lat || !lon || !window.SunCalc) return;
  ...
  for (let d = new Date(first); d <= last; d.setDate(d.getDate() + 1)) {
    ...
    if (day === 5) {
      ...
      const friTimes = SunCalc.getTimes(fri, lat, lon);
      const satTimes = SunCalc.getTimes(sat, lat, lon);

      const sunsetFri = friTimes.sunset || new Date(fri.getFullYear(), fri.getMonth(), fri.getDate(), 18, 0);
      const duskSat = satTimes.dusk;
      const nautSat = satTimes.nauticalDusk;
      let tzeitSat;
      if (duskSat && nautSat) {
        tzeitSat = new Date((duskSat.getTime() + nautSat.getTime()) / 2);
      } else if (duskSat) {
        tzeitSat = new Date(duskSat.getTime() + 15 * 60000);
      } else {
        tzeitSat = new Date(satTimes.sunset.getTime() + 35 * 60000);
      }

      const inDate = new Date(sunsetFri.getTime() - 20 * 60000); // 20 דקות לפני שקיעה
      const outDate = new Date(tzeitSat.getTime() + 40 * 60000); // 40 דקות אחרי צאה"כ משוער

      const inKey = dateKey(fri);
      const outKey = dateKey(sat);

      map[inKey] = map[inKey] || {};
      map[inKey].in = fmtTime(inDate);

      map[outKey] = map[outKey] || {};
      map[outKey].out = fmtTime(outDate);
    }
  }
...
async function fetchHolidaysForYear(year) {
  ...
  const base = `https://www.hebcal.com/hebcal/?v=1&cfg=json&maj=on&min=on&mod=on&year=${year}&geo=pos&latitude=${lat}&longitude=${lon}&lg=he`;
...
function createDayCell(d, otherMonth) {
  ...
  if (sh?.in) {
    ...
    tagIn.className = "tag shabbat";
    tagIn.innerHTML = `🕯️ ${sh.in}`;
    tags.appendChild(tagIn);
  }
  if (sh?.out) {
    const tagOut = document.createElement("div");
    tagOut.className = "tag shabbat";
    tagOut.innerHTML = `✨ ${sh.out}`;
    tags.appendChild(tagOut);
  }

  const holiday = CAL.holidays[key];
  if (holiday) {
    const tagH = document.createElement("div");
    tagH.className = "tag holiday";
    tagH.textContent = holiday.name;
...
function openDayModal(dateStr) {
  ...
  const holidayLabel = document.getElementById("dayModalHoliday");
  const shInfoBox = document.getElementById("dayShabbatInfo");
  ...
  const holiday = CAL.holidays[dateStr];
  holidayLabel.textContent = holiday ? `🎉 ${holiday.name}` : "";

  const sh = CAL.shabbatMap[dateStr];
  if (sh?.in || sh?.out) {
    const parts = [];
    if (sh.in) parts.push(`🕯️ כניסת שבת/חג: ${sh.in}`);
    if (sh.out) parts.push(`✨ יציאה: ${sh.out}`);
    shInfoBox.textContent = parts.join(" • ");
  } else {
    shInfoBox.textContent = "";
  }
  ...
  const routine = getRoutineEventsForDate(dateStr);
  const userEvents = getEventsForDate(dateStr);
  const dayEvents = [...routine, ...userEvents].sort((a, b) =>
  ...
  // weather inline – רק אם אין אירועים / משימות של המשתמש
  const userHasEvents = userEvents.filter(
    (ev) => ev.kind !== "routine-work" && ev.kind !== "routine-food"
  ).length > 0;

  if (!userHasEvents) {
    weatherInline.textContent = "🌤 טוען מזג אוויר...";
    fetchWeatherForSelectedDay(true).catch(() => {
...
// ---------- Weather (Open-Meteo) ----------
async function fetchWeatherForSelectedDay(inline = false) {
  const modalContent = document.getElementById("weatherContent");
  const inlineBox = document.getElementById("dayWeatherInline");
  if (!inline && modalContent) modalContent.textContent = "טוען מזג אוויר...";
  ...
  try {
    ...
    if (inline && inlineBox) {
      inlineBox.innerHTML = `${descEmoji} · ${tMax}° / ${tMin}°${
        typeof rainProb === "number" ? ` · ☔ ${rainProb}%` : ""
      }`;
    ...
    if (inline && inlineBox) inlineBox.textContent = "";
    if (!inline && modalContent) modalContent.textContent = "תקלה בטעינת מזג האוויר.";
  }
}
...
// ---------- Search ----------
function searchEventsByText(query) {
  const q = query.trim().toLowerCase();
  if (!q) return [];
  const results = [];
  for (const [dateStr, bucket] of Object.entries(CAL.events)) {
    for (const ev of Object.values(bucket)) {
      if (ev.kind === "routine-work" || ev.kind === "routine-food") continue;
      const haystack = [
        ev.title || "",
        ev.address || "",
        ev.kind || "",
...
function initUI() {
  ...
  document.getElementById("weatherDayBtn").addEventListener("click", () => {
    ...
    fetchWeatherForSelectedDay(false);
  });
  ...
  searchRunBtn.addEventListener("click", () => {
    ...
    searchResults.innerHTML = "";
    if (!q.trim()) {
      searchStatus.textContent = "נא להזין טקסט לחיפוש.";
      return;
    }
    const found = searchEventsByText(q);
    if (!found.length) {
      searchStatus.textContent = "לא נמצאו תוצאות.";
      return;
    }
    searchStatus.textContent = `נמצאו ${found.length} תוצאות:`;
    found.slice(0, 80).forEach(({ dateStr, ev }) => {
      const div = document.createElement("div");
      div.className = "search-item";
      div.innerHTML = `
        <div><strong>${ev.title}</strong></div>
        <div class="hint">${dateStr} · ${ev.start || ""}</div>
      `;
      div.addEventListener("click", () => {
        CAL.viewYear = fromKey(dateStr).getFullYear();
        CAL.viewMonth = fromKey(dateStr).getMonth();
        renderMonth();
        computeShabbatForMonth(CAL.viewYear, CAL.viewMonth);
        fetchHebrewForMonth(CAL.viewYear, CAL.viewMonth);
